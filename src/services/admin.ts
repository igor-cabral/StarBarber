...
export async function deleteService(id: string) {
  ...
  if (error) {
    if (error.code === '23503') {
      throw new Error(
        'Este serviço já possui agendamentos vinculados e não pode ser excluído. Desative-o em vez de excluir.'
      );
    }
    throw error;
  }
...
export async function deleteBarber(id: string) {
  ...
  if (error) {
    if (error.code === '23503') {
      throw new Error(
        'Este profissional já possui agendamentos vinculados e não pode ser excluído. Desative-o em vez de excluir (assim ele some da área pública, mas o histórico é mantido).'
      );
    }
    throw error;
  }
