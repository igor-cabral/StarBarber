...
export function RescheduleModal({
  ...
  async function choose(slotStart: string) {
    ...
    try {
      ...
      const ends = new Date(new Date(slotStart).getTime() + duration * 60000).toISOString();
      await rescheduleAppointment(appointment.id, slotStart, ends);
      onDone();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <Card className="w-full max-w-md p-6">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="font-display text-lg font-semibold">Remarcar agendamento</h2>
        ...
        <p className="mb-3 text-sm text-graphite">
          {appointment.customer?.name} · {appointment.service?.name} · {appointment.barber?.name}
        </p>
